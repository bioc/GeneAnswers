...
\author{Lei Huang$^\ddagger$\footnote{lhuang7@uchicago.edu}, Xishu Wang$^\ddagger$\footnote{wangxishu@gmail.com}, Jing Wen$^\ddagger$\footnote{vivannawj@gmail.com}, Gang Feng$^\ddagger$\footnote{g-feng@northwestern.edu}}
\begin{document}
\setkeys{Gin}{width=1\textwidth} 

\title{getListGIF module }
\maketitle
\begin{center}$^\ddagger$Center for Research Informatics \\ University of Chicago, Chicago, IL, 60637, USA
\end{center}
...
\begin{description}
  ...
  \item \emph{background}: This is the argument for the output word cloud png file background. Only white and black are supported. Default is white.
  \item \emph{type}: This is the argument for the analysis which ListGIF can perform on the query gene list. User can specify either \texttt{genelevel}, \texttt{riflevel} or \texttt{goterm} enrichment analysis. 
  \begin{itemize}
...
\section{Example}
The following is an example to obtain the ListGIF word-cloud graph for gene list: MRPS35, NBL1, PSMD14, PGK1, SMC4, SLC16A1 and CAV1.
