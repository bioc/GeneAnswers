...
Also the new version GeneAnswers integrates gene interaction database from NCBI. The following case is a typical one to show interaction information could be included in basic concepts-genes network.
