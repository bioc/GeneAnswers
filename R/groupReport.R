...
function(dataMatrix, gAList, topCat=10, methodOfCluster=c('mds', 'sort'), matrixOfHeatmap=NULL, clusterTable=c('geneNum', 'pvalue', NULL), catTerm=TRUE, 
    ...
    if (catType != 'Unknown') { 
		tableNames <- c('Concepts-Genes Table', 'Concepts Relation', paste('Group', colnames(dataMatrix), 'Concepts-Gene network'), gsub('Genes / Group :: NA and ', replacement='', 
	...
	for (i in 1:length(tableNames)) {
		if (attr[i] != 'Pictures') {
			...
			if (i == 2) {
				...
				png(filename=tempConceptsRelationFileName, width=1000, height=1000)
				.catsCluster(dataMatrix[c(1:(dim(dataMatrix)[1]-1)),], gAList, catType=catType, nameLength=94, addID=TRUE)
				dev.off()
				cat(paste('<H2 align=center><font face="courier" size="2"><A name="', paste('h-', i, sep=''), '">', tableNames[i], "</a></font></H2>", sep=''), file = outFile, sep = "\n") 
				cat('<center><IMG src="', tempConceptsRelationFileName, '"></center>\n', file = outFile, sep="")
			} else {
				...
				if (!is.null(drawCats)) {
					...
					if (length(colorValueColumn) > 1) geneAnswersConceptNet(gAList[[i-2]], centroidSize='pvalue', colorValueColumn = colorValueColumn[i-2], output='fixed', showCats=drawCats, catTerm=catTerm, geneSymbol=TRUE)
