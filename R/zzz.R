	...
	colorBar=FALSE, colorBarLabel=NULL, mapType=c('table', 'heatmap', 'none'),  ...)
{
	...
	if (length(colorMap) == 1) {
		if (tolower(colorMap[1]) == 'default') {
			colorMap <- .defaultHeatmapColors(ncols=256)
		} else {
	...
	if (mapType != 'none') {
		if (mapType == 'table') .drawTable(dataMatrix, addRowLabel=addRowLabel, mar=mar, cex.axis=cex.axis)
			...
			if (colorBar) {
				if (rotate) layout(matrix(c(1,2), nrow=1), widths=c(2.5,0.4))
			    else layout(matrix(c(1,2), ncol=1), heights=c(2.5,0.4))
			}
			oldsetting <- par('mar'=mar)
			if (symmetry) {
			...
			if (colorBar) {
				...
				if (rotate) {
					par(mar=c(6,1,6,3))
					colorBar <- matrix(color, nrow=1)
					image(1:nrow(colorBar), 1:ncol(colorBar), colorBar, col=colorMap, axes=FALSE, ylab="", xlab="")
				} else {
					par(mar=c(3,6,1,6))
					colorBar <- matrix(color, ncol=1)
					image(1:nrow(colorBar), 1:ncol(colorBar), colorBar, col=rev(colorMap), axes=FALSE, ylab="", xlab="")
				}
				...
				if (relative) {
					...
					if (rotate) axis(4, at = c(1.5, length(colorMap)-0.5), labels=colorBarLabel, tick=FALSE)
					else axis(1, at = c(length(colorMap)-0.5, 1.5), labels=colorBarLabel, tick=FALSE)			
				} else {
					if (is.null(colorBarLabel)) {
					...
					if (rotate) axis(4, at = seq(1, length(colorMap), length=length(colorBarLabel)), labels=colorBarLabel)
					else axis(1, at = seq(1, length(colorMap), length=length(colorBarLabel)), labels=colorBarLabel)
				}
				box()
				#dev.set(deviceNo)
				layout(matrix(1))
...
.drawHTMLtable <- function(dataMatrix, outFile, tableName='Table', tableLink= NULL, externalLinkOfTable = NULL, tableCenter=TRUE, catType=c('GO', 'KEGG', 'Entrez', 'DOLITE', 'REACTOME.PATH', 'Unknown'), 
